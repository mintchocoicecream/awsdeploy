...
# 최초 배포, 업그레이드 배포
# 업그레이드 배포시 명령
# 코드 수정 -> 로컬 테스트 완료 -> git커밋, push -> fab deploy
# $> fab deploy
def deploy():
    # 저장소에 가서 최신 소스로 서버상태에 반영한다.(git 연동)
    _get_latest_source()
    # 생략
    # _put_envs()
    # 가상환경 업데이트
    _update_virtualenv()
    # 서버가 가동될 때 홈디렉토리, 로그파일의 위치등 설정
    # *.conf가 수정 및 생성된다
    # wsgi.py 앤트래포인트가 설정되어있다
    _make_virtualhost()
...
def _get_latest_source():
    if exists(project_folder + '/.git'):
        # 저장소에 가서 추가되 내용을 패치해온다.
        run('cd %s && git fetch' % (project_folder,))
    else:
        # .git가 해당 프로젝트 안에 없으면 저장소에 가서 프로젝트내용을 가져온다.
